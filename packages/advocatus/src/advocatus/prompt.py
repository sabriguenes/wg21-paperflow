...
"""Parse step metadata from ``advocatus.md`` and build the pipeline.

``advocatus.md`` is the upstream authority for pipeline structure. Each
...
user knows to go fix ``advocatus.md``.
"""
...
class StepSpec:
    ...
    ``advocatus.md`` is the upstream authority for pipeline structure.
...
def build_pipeline(
    ...
    """Parse ``advocatus.md`` metadata, attach hooks, return ordered specs.
    ...
    if orphan_hooks:
        raise HookMismatchError(
            f"Hooks registered for steps not in advocatus.md: "
    ...
    if missing_hooks:
        raise HookMismatchError(
            f"Steps in advocatus.md have no registered hooks: "
