...
"""CLI command module for 'paperflow advocatus'."""
...
def command(args: argparse.Namespace, backend: StorageBackend) -> int:
    from advocatus import AdvocatusError, advocatus_paper, advocatus_since
    ...
    if _MONTH_RE.match(target):
        progress_ctx, on_progress = make_progress_handler("Batch advocatus")
        with progress_ctx:
            results = asyncio.run(
                advocatus_since(
                    target, backend,
        ...
        print(
            f"Batch advocatus: {ok} succeeded, {failed} failed "
    ...
    progress_ctx, on_progress = make_progress_handler("Examining")
    ...
    with progress_ctx:
        ...
        except AdvocatusError as exc:
            print(f"Advocatus failed: {exc}", file=sys.stderr)
            return 1
        except pydantic_ai.exceptions.UsageLimitExceeded as exc:
            print(
                f"Advocatus aborted: LLM usage limit reached ({exc})",
                file=sys.stderr,
            )
            return 1
        except Exception as exc:  # last-resort firewall: log full chain
            msg = f"Advocatus failed unexpectedly: {type(exc).__name__}: {exc}"
            cause = exc.__cause__
            while cause:
    ...
    if stop_after is not None:
        # advocatus_paper returned the partial trace string; CLI writes it.
        trace_path = backend.get_trace_md_path(pid, "advocatus")
        trace_path.write_text(relatio, encoding="utf-8")
    ...
    if not out_path.exists() or out_path.stat().st_size == 0:
        print(
            f"Advocatus write failed: {out_path} is empty or missing",
            file=sys.stderr,
        )
        return 1

    print(f"Relatio written to {out_path}")
    return 0
