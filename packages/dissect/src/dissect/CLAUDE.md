...
`dissect.md` is the upstream authority for pipeline structure. It defines the step sequence, step metadata (tools, conditions, system prompts), and all LLM-facing instructions. Python conforms to it. `build_pipeline()` validates the conformance at startup. Agent assignment (which model backend handles which step) is declared in Python (`pipeline.py`), not in `dissect.md`.
...
- `pipeline.py` - async orchestration: `StepContext`, hook registry (`_HOOKS`), generic runner (`_run_agent`), dispatch loop, and the public `dissect_paper()` / `dissect_since()` entry points. Hooks are small prepare/extract functions (~10-20 lines each); the custom hooks for Steps 9 and 10 are longer because they run multiple sub-LLM calls per step.
...
- `harness.py` - pure Python: paper chunking, SourceLoc computation, deterministic dedup (tiers 0-1, tier 2 grouping helper), citation extraction, plus pysbd sentence decomposition and the Step 1 (Tag Sentences) classifier-facing helpers (`_decompose_sentences`, `_tag_sentences`, `_render_tagged_chunk`, `_split_tagged_by_chunk`). Imports `ClassifierBackend` for type but never names a model. No LLM, no paperstore imports, no network I/O.
...
- `triage.py` - pure Python embedding helpers used by Steps 9 and 10. Reuses the `shadow._load_model()` singleton (no second model load). Provides `embed_claims`, `embed_evidence`, `cosine_matrix`, `top_k_per_row`, `above_threshold_pairs`, `centrality_scores`, `tier_split`, `interleave_propositions`. No LLM calls, no network I/O.
...
```
 ...
 1. Tag Sentences     decompose + classify each sentence      (pure Python + local classifier)
 2. Extract Claims    normative claims per chunk              (parallel LLM)
 3. Dedup Claims      deterministic tiers 0-1-2 + shadow      (pure Python)
 4. Extract Evidence  supporting evidence per chunk           (parallel LLM)
 5. Dedup Evidence    deterministic tiers 0-1 + shadow        (pure Python)
 6. Extract Factual   factual claims per chunk                (parallel LLM)
 7. Dedup Factual     deterministic tiers 0-1-2               (pure Python)
 8. Extract Rhetoric  rhetoric markers per chunk              (parallel LLM)
 9. Verify            custom: triage + per-batch propositions + per-pair disclaim
10. Load-Bearing      custom: deterministic classify + per-claim binary
11. Verify Citations  fetch and verify each cited paper       (parallel LLM + web_fetch)
12. Web Search        search for evidence on critical gaps    (parallel LLM + web_search/fetch)
13. Resolve External  integrate external evidence             (single LLM)
14. Caput Causae      identify the load-bearing root cause    (single LLM)
15. Detect Patterns   cross-rhetoric pattern analysis         (single LLM)
16. Report            render final dissect markdown           (pure Python)
```

Three named agents own all LLM calls:

| Agent | Slot | Steps | Config |
|---|---|---|---|
| `extraction_agent` | `fast` | 2, 4, 6, 8 | `thinking_budget=2048` |
| `synthesis_agent` | `default` | 9, 10, 13, 14, 15 | `thinking_budget=4096` |
| `research_agent` | `tool` | 11, 12 | tools-capable required |

Step 1 (Tag Sentences) uses `ctx.classifiers['selector']` -- a local zero-shot text classifier configured by `[classifiers.NAME]` in `SERVICES.toml` -- instead of an agent. No LLM call.

Steps 0, 1, 3, 5, 7, 16 are pure Python (no LLM calls).

The generic runner (`run_agent`) handles system prompt composition, tool registration, and debug logging. Tool registration is prompt-driven: the runner reads `meta.tools` and looks up each name in `ctx.tool_registry`.

Hooks split responsibility: `prepare` formats state fields into the user message; `extract` stores the LLM output back into state. Agent assignment is declared in `_build_hooks()` in `pipeline.py`.
...
1. Add a `## N. Name` section to `dissect.md` with the metadata block (optionally `**Tools:**`, `**Condition:**`, `**System prompt:**`).
2. Define a Pydantic output model in `models.py`.
3. Write `_prepare_<name>` and `_extract_<name>` hooks in `pipeline.py`.
4. Register in `_build_hooks()` under the exact section header text, assigning the appropriate agent.
5. Add fields to `PipelineState` if the step writes new state.

## Invariants

- `dissect.md` is the authority for LLM-facing instructions, tool declarations, conditions, and system prompts. Agent assignment (which backend handles which step) is declared in `_build_hooks()` in `pipeline.py`.
- Step 1 (Tag Sentences) reads `ctx.classifiers['selector']` -- a local zero-shot text classifier configured by `[classifiers.NAME]` in `SERVICES.toml` and resolved via `pipeline.resolve_classifier_slots`. `state.tagged_sentences` is ephemeral: populated per run, not persisted; consumed only by the renumbered Step 2 (Extract Claims) via `_prepare_extract_claims_chunks`. When no classifier is configured, Step 1 leaves the field at `None` and Step 2 falls back to the untagged chunk render.
- pysbd is a hard dependency for Step 1. The decomposer is rule-based and deterministic by construction; consistent Step 2 input quality depends on it.
- Defense-in-depth for SourceLoc: `_strip_line_prefix` strips both the line-number prefix (``N|``) AND any leading classifier tag (``[TARGET]`` / ``[CONTEXT]``) the LLM might accidentally copy into its emitted `text` field. The Step 2 prompt instructs the LLM not to include tags in `text`; this regex is the safety net so SourceLoc computation always sees a verbatim span.
...
- Steps 9 and 10 are custom hooks owning their own LLM choreography.
  - Step 9 (Verify) runs five phases: embedding triage, centrality scoring + tier split, batched (claim, evidence) verify propositions over Tier 1 claims only, per-pair disclaim detection over cosine-filtered claim pairs, then aggregation into `state.verdicts`. Tier 2 claims default to `unproven` with no LLM call.
  - Step 10 (Load-Bearing) runs a deterministic classification pass over the verdicts and dependency graph, then one short LLM binary call per Tier 1 claim asking whether the claim is load-bearing. Tier 2 claims auto-classify as `peripheral`. A second pass downgrades `anchored` claims whose dependencies are `conflicted` or `critical_gap` into `depends_on_contested`.
