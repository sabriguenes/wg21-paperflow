...
f(reflexpr(<del>"not an int"</del>)); <ins>// ko</ins>
...
# define log(message) \\
if(enable_logging) { std::cout << message << <del>"\n"</del> };
...
constexpr_if(
true,
reflexpr(print(<del>"Hello, world!"</del>)),
reflexpr(print(<del>"not evaluated so nothing gets printed"</del>))
);
}
...
<del>"Include 'advanced' options in the menu listing"</del>);
ABSL_FLAG(std::string, output_dir, <del>"foo/bar/baz/", "output file dir"</del>);
ABSL_FLAG(std::vector<std::string>, languages,
std::vector<std::string>({<del>"english", "french", "german"</del>}),
<del>"comma-separated list of languages to offer in the 'lang' menu"</del>);
ABSL_FLAG(absl::Duration, timeout, absl::Seconds(30), <del>"Default RPC deadline"</del>);
...
-> namespace :: {
void* unqualid(<del>"AbslFlagsInitFlag"</del>, name) {
return absl::flags_internal::MakeFromDefaultValue<T>(exprid(default_value));
}
constinit absl::Flag<Type> unqualid(<del>"ABSEIL_FLAGS_"</del>, name) {
name,
source_location::current().file_name(),
&unqualid(<del>"AbslFlagsInitFlag"</del>, name),
help
};
};
}
...
f(std::fwd!(args));
std::assert!(1+1 == 2, <del>"Maths are broken"</del>);
catch2::require!(vec.empty());
abseil::RegisterFlag<bool>!(<del>"ftime-trace", false, "Profile compile time"</del>);
