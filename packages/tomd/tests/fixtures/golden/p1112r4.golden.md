...
| `struct cell {    int idx;    double fortran_input;    double fortran_output;};static_assert(std::is_standard_layout_v<cell >);` | `struct layout(standard) cell {    int idx;    double fortran_input;    double fortran_output;};` |
| --- | --- |
| `// trick to simulate extension#define CELL_MEMBERS \    int idx; \    double fortran_input; \    double fortran_output;` | `struct layout(standard) cell {    int idx;    double fortran_input;    double fortran_output;};` |
| `struct cell {   CELL_MEMBERS};static_assert(std::is_standard_layout_v<cell >);` | `struct layout(standard) cell_ex : cell {   int extra_info;};// now works naturally and IS standardlayouttoo; identical to left` |
