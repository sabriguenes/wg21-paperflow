...
| --- | --- | --- | --- |
| Abstraction | `struct Draw : std::dispatch<void()> {   template <class T>   void operator()(const T& self) { self.Draw(); } }; struct Area : std::dispatch<double()> {   template <class T>   double operator()(const T& self) { return self.Area(); } }; struct FDrawable : std::facade<Draw, Area> {};  class Rectangle {` |  | `struct IDrawable {   virtual void Draw() const = 0;   virtual double Area() const = 0;   virtual ~IDrawable() {} };  class Rectangle : public IDrawable {` |
| Implementation | `public:   void Draw() const {     printf("{Rectangle: width = %f, height = %f}", width_, height_);   }   double Area() const { return width_ * height_; }   private:   double width_;   double height_; };` |  | `public:   void Draw() const override {     printf("{Rectangle: width = %f, height = %f}", width_, height_);   }   double Area() const override       { return width_ * height_; }   private:   double width_;   double height_; };` |
| Invocation | `void DoSomethingWithDrawable(std::proxy<FDrawable> p) {   p.invoke<op::Draw>(); }` | Table 2 – Sample code to compile | `void DoSomethingWithDrawable(std::unique_ptr<IDrawable> p) {   p->Draw(); }` |
...
| Client side `mov     rax, qword ptr [rdi + 8]` | `movsd   xmm0, qword ptr [rax] movsd   xmm1, qword ptr [rax + 8] mov     edi, offset .L.str.18 mov     al, 2 jmp     printf` | Table 4 – Generated code from clang 13.0.0 (x86-64) | `movsd   xmm0, qword ptr [rdi + 8] movsd   xmm1, qword ptr [rdi + 16] mov     edi, offset .L.str mov     al, 2 jmp     printf` |

|  | The "proxy" | Inheritance-based polymorphism |
| --- | --- | --- |
| Library side `ldr     x1, [x0], 8` |  | `ldr     x0, [x0]` |
|  | `ldr     x1, [x1, 24]` | `ldr     x1, [x0]` |
|  | `mov     x16, x1` | `ldr     x1, [x1]` |
|  | `br      x16` | `mov     x16, x1` |
|  |  | `br      x16` |
| Client side `mov     x1, x0` |  | `mov     x1, x0` |
|  | `adrp    x0, .LC3` | `adrp    x2, .LC0` |
|  | `add     x0, x0, :lo12:.LC3` | `add     x0, x2, :lo12:.LC0` |
|  | `ldr     d0, [x1]` | `ldp     d0, d1, [x1, 8]` |
|  | `b       printf` | `b       printf` |
|  | The "proxy" | Inheritance-based polymorphism |
| Library side | `ld      a1, 0(a0)` | `ld      a0, 0(a0)` |

Table 5 – Generated code from gcc 11.2 (ARM64)

| Library side `ld      a1, 0(a0)` |  | `ld      a0, 0(a0)` |
| --- | --- | --- |
|  | `ld      a5, 24(a1)` | `ld      a1, 0(a0)` |
|  | `addi    a0, a0, 8` | `ld      a5, 0(a1)` |
|  | `jr      a5` | `jr      a5` |
| Client side `ld      a0, 8(a0)` |  | `ld      a2, 16(a0)` |
|  | `ld      a2, 8(a0)` | `ld      a1, 8(a0)` |
|  | `ld      a1, 0(a0)` | `lui     a0, %hi(.L.str)` |
|  | `lui     a0, %hi(.L.str.18)` | `addi    a0, a0, %lo(.L.str)` |
|  | `addi    a0, a0, %lo(.L.str.18)` | `tail    printf` |
...
| Name | Value |
| --- | --- |
| `template <class P, class... Args>` | `conditional_t<proxiable<P, F>,` |
| `HasNothrowPolyConstructor<P, Args...>` | `is_nothrow_constructible<P, Args...>, false_type>::value` |
| `template <class P, class... Args>` | `conditional_t<proxiable<P, F>,` |
| `HasPolyConstructor<P, Args...>` | `is_constructible<P, Args...>, false_type>::value` |
...
| `template <class P, class... Args>` | `HasNothrowPolyConstructor<P, Args...> &&` |
| --- | --- |
| `HasNothrowPolyAssignment` | `HasNothrowDestructor` |
| `template <class P, class... Args>` | `HasPolyConstructor<P, Args...> &&` |
| `HasPolyAssignment` | `HasDestructor` |
...
| --- | --- | --- |
| `rhs` **contains a value** Swap the contained values of `*this` and | `rhs` with a temporary storage. If an exception is thrown, each of `*this` and `rhs` is in a valid state with unspecified value. | Equivalent to (`*this` `=` `std::move(rhs)`); post condition is that `*this` contains a value and `rhs` does not contain a value. no effect |
| `rhs` **does not contain** | Equivalent to (`rhs` `=` |  |
| **a value** | `std::move(*this)`); post condition is that `*this` does not contain a value and `rhs` contains a value. |  |
