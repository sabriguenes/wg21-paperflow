...
- Bold-only headings (no section number, no enlarged font, not a known name) are accepted at LOW confidence when the first line is short (<= `_HEADING_MAX_WORDS`) and does not contain code-like characters (`{}();=<>[]`) or monospace spans. Level is inferred from context (`last_heading_level + 1`, minimum 3). This handles WG21 papers that use bold-at-body-size sub-headings.
...
- The order is enforced by `_canonicalize_front_matter` in `api.py`,
  ...
  `lib/metadata_yaml/format.py`, the single source of truth for ordering
  (`FRONT_MATTER_ORDER`).
...
- `lib/__init__.py` - Re-exports from `lib/shared.py` and `lib/metadata_yaml/format.py` for backward compatibility.
- `lib/shared.py` - Shared text utilities and constants for PDF and HTML converters: `ascii_escape`, `strip_format_chars`, `normalize_date` (multi-format date parser), `parse_author_lines`, `ALLOWED_LINK_SCHEMES`, shared regex patterns (`EMAIL_RE`, `DATE_RE`, `DOC_NUM_RE`, `SECTION_NUM_PREFIX_RE`), and their reusable shape strings (`DOC_NUM_PATTERN`, `SECTION_NUM_PATTERN`) consumed by `lib/pdf/types.py` to build `DOC_FIELD_RE` and `SECTION_NUM_RE`.
- `lib/metadata_yaml/` - Metadata extraction and YAML front matter formatting. `format.py` is the single source of truth for `FRONT_MATTER_ORDER` (`title, document, date, intent, audience, reply-to`) and `format_front_matter`. `extract.py` contains metadata field extraction from PDF sections, email safety-net fallback (`enrich_pdf_reply_to`), PDF metadata fallbacks (`apply_pdf_metadata_fallbacks`), and revision override (`override_revision_from_filename`). `strip.py` removes page-0 headings duplicating front-matter data (`strip_metadata_headings`), pre-heading metadata fragments (`strip_pre_heading_fragments`), and pre-content paragraphs (`strip_pre_content_paragraphs`).
- `lib/body/` - Body content processing. `abstract.py` contains abstract heading deduplication (`dedup_abstract`), metadata-echo removal from UNCERTAIN sections (`strip_metadata_from_uncertain`), and abstract reordering in UNCERTAIN sections for two-column PDFs (`reorder_abstract_in_uncertain`).
...
- `lib/pdf/pipeline.py` - PDF conversion pipeline orchestrator. `_run_pipeline()` enforces explicit phase ordering: Phase 1 (metadata extraction via `metadata_yaml.extract`), Phase 1b (body structuring via `structure_body`), Phase 1c (metadata fallbacks and enrichment via `metadata_yaml.extract`), Phase 1d (metadata stripping via `metadata_yaml.strip`), Phase 2 (TOC stripping, abstract dedup, body cleanup via `body.abstract`), Phase 3 (emit). Contains no inline metadata logic; delegates to `metadata_yaml/` and `body/` modules.
...
- `lib/pdf/table.py` - Three-pass table detection. Pass 1: MuPDF block/line columnar layout (x-gap > 50) with geometric column profile (x positions co-occurring in the same y-band across 2+ rows). Orphan absorption for wrapped cell first lines (same-page only). Pass 2: side-by-side table detection for tables where each cell is a separate MuPDF block (e.g. Tony Tables with multi-line code cells); clusters body block x-positions into columns, groups by y-overlap into rows, requires at least 2 valid data rows. Pass 3: geometric column grouping for tables where MuPDF distributes columns across separate single-column blocks (e.g. schedule tables with dates in left blocks and descriptions in right blocks, or multi-column comparison tables with each cell as its own block); uses confirmed column positions from `_find_column_xs`, groups blocks into rows by y-overlap, then expands by absorbing adjacent mixed-column blocks at line level. Extracts as high-confidence TABLE sections, excludes table regions from spatial path.
...
- `lib/pdf/emit.py` - Markdown generation (headings, paragraphs, code blocks, tables, nested lists) with span-level formatting (inline code, bold, italic, links). Skips headings that render as bare ATX prefixes with no text content. Table cell rendering consolidates all-monospace spans into single backtick pairs. Wording section rendering preserves line breaks for code content, collapses prose to single lines. Prompts file generation for uncertain regions.
...
`$env:WG21_DATA_DIR` must point to the directory that **contains** `paperstore.db` and `paperstore/`.

**On this machine:** `C:\Users\sabog\Desktop\cppalliance\cppalliance\data\paperstore`

```powershell
$env:WG21_DATA_DIR = "C:\Users\sabog\Desktop\cppalliance\cppalliance\data\paperstore"
```

Do NOT point it one level deeper into `paperstore/paperstore/`. If the path is wrong, `paperflow convert` reads from and writes to a nested subdirectory that the rest of the tooling does not see: conversions appear to succeed but the output files land in the wrong place.
