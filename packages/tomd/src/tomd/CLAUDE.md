...
- `lib/__init__.py` - Shared text utilities and constants for PDF and HTML converters: `ascii_escape` (kept for external use, no longer called in pipeline), `strip_format_chars`, `format_front_matter` (emits keys in the strict canonical order from `FRONT_MATTER_ORDER`: `title, document, revision, date, intent, audience, reply-to`, with unknown keys after `audience` so `reply-to` stays last), `parse_author_lines` (with deobfuscation fallback), `deobfuscate_email` (two families: word-based "at"/"dot" and underscore-based `_at_`, both EMAIL_RE-gated), `ALLOWED_LINK_SCHEMES`, shared regex patterns (`EMAIL_RE`, `DATE_RE`, `DOC_NUM_RE`, `SECTION_NUM_PREFIX_RE`), and their reusable shape strings (`DOC_NUM_PATTERN`, `SECTION_NUM_PATTERN`) consumed by `lib/pdf/types.py` to build `DOC_FIELD_RE` and `SECTION_NUM_RE`.
- `lib/similarity.py` - Dual-algorithm string similarity (SequenceMatcher + Jaccard). Per-algorithm thresholds, 200-char circuit breaker. `fuzzy_match_label(candidate, known, threshold)` matches a candidate string against a known set using symmetric SequenceMatcher (max of both argument orders); used by PDF `wg21.py` and HTML `extract.py` for typo-tolerant metadata label recovery. Format-agnostic.
...
- `lib/html/extract.py` - Generator detection (mpark, bikeshed, hand-written, hackmd, wg21, schultke, dascandy/fiets, unknown). Per-generator metadata extraction. `_FIELD_SYNONYMS` + `_match_field` provide a shared label matcher reused across extractors: exact synonym lookup first, then fuzzy fallback via `similarity.fuzzy_match_label`. Boilerplate stripping per generator. Eight generator families supported. Schultke detected by `<code-block>` custom element presence; dascandy/fiets by meta generator tag. `_deobfuscate_email` reverses "at"/"dot" anti-spam patterns (EMAIL_RE-gated). `_extract_plaintext_authors` provides fallback author extraction when mailto links are absent (plain-text emails, deobfuscated emails, bare names).
